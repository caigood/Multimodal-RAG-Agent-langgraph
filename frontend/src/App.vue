...
const pageMeta = {
  dashboard:          { title: '首页', sub: '企业知识库工作台' },
  chat:               { title: '智能对话', sub: '基于大模型的知识问答' },
  'kb-categories':    { title: '类目管理', sub: '管理文档分类与归档' },
  'admin-collections':{ title: '知识库列表', sub: '管理向量知识库' },
  'admin-create':     { title: '创建知识库', sub: '新建知识库集合' },
  'admin-config':     { title: '配置信息', sub: '查看系统运行配置' },
  devtools:           { title: '开发工具', sub: '接口调试与运行检查' },
}
...
onMounted(async () => {
  try {
    ...
    availableModels.value = [
      { name: 'qwen3.7-plus' }, { name: 'kimi-k2.6' }
    ]
  }
})
</script>
